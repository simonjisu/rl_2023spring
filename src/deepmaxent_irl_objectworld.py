...
def run_deepmaxent_irl(args, init_start_pos=None):
    ...
    if init_start_pos is None:
        trajs = generate_demonstrations(ow, policy_gt, 
        ...
        if isinstance(init_start_pos[0], list) or isinstance(init_start_pos[0], tuple):
            ...
            for sp in init_start_pos:
                t = generate_demonstrations(ow, policy_gt, 
                                            n_trajs=args.n_query, 
                                            len_traj=args.l_traj, 
                                            rand_start=False, 
                                            start_pos=sp)
    ...
    while True:
        ...
        if args.active:
            ...
            rewards_new_T, values_new, policy_new = finite_policy_iteration(P_a, policy, ow, args.gamma, args.l_traj)
            # acquistion process
            ...
            for sp in start_points_new:
                t_new = generate_demonstrations(ow, policy_gt, 
                                                n_trajs=1, 
                                                len_traj=args.l_traj, 
                                                rand_start=False, 
                                                start_pos=sp)
            ...
            trajs_new = generate_demonstrations(ow, policy_gt, 
                                                n_trajs=args.n_query, 
                                                len_traj=args.l_traj, 
                                                rand_start=True, 
                                                start_pos=None)
